...
# ==========================================
# Main Execution: Full Period Backtest (Compounding)
# ==========================================

if __name__ == '__main__':
    files = [
        'ETHUSDT_2017_1h.csv', 'ETHUSDT_2018_1h.csv', 'ETHUSDT_2019_1h.csv',
        'ETHUSDT_2020_1h.csv', 'ETHUSDT_2021_1h.csv', 'ETHUSDT_2022_1h.csv',
        'ETHUSDT_2023_1h.csv', 'ETHUSDT_2024_1h.csv', 'ETHUSDT_2025_1h.csv',
        'ETHUSDT_2026_1h.csv'
    ]
    ...
    modes = {
        'Scheme_A': {'mode': 'A', 'vol_mult': 2.4, 'p_tp1_long': 0.2, 'desc': 'Ultra-low frequency'},
        'Scheme_B': {'mode': 'B', 'vol_mult': 1.8, 'p_tp1_long': 0.3, 'desc': 'Maximum explosive'},
        'Scheme_C': {'mode': 'C', 'vol_mult': 2.4, 'p_tp1_long': 0.2, 'desc': 'Adaptive Optimal Hybrid'}
    }
    ...
    for name, config in modes.items():
        ...
        bt = Backtest(df, Sentinel5Adaptive, cash=10000, commission=0.0004)
        stats = bt.run()
        results[name] = stats
        
        # Save Scheme_C trades to full_period_trades.csv by default
        if name == 'Scheme_C':
            stats['_trades'].to_csv('full_period_trades.csv')

    print("\n" + "="*80)
    print("=== Long-term Compounding Results Comparison (2017-2026) ===")
    print("="*80)
    
    metrics = [
        ('Start Date', lambda s: s['Start']),
        ('End Date', lambda s: s['End']),
        ('Initial Capital', lambda s: '$10,000'),
